...
/**
 * StegaVault - Collaborator Login Page
 * File: collaborator/login.php
 */

session_start();
require_once '../includes/config.php';

// If already logged in, redirect to collaborator dashboard
if (isset($_SESSION['user_id'])) {
    // If admin tries to use collaborator login, redirect to admin
...
<head>
    ...
    <title>Collaborator Login - StegaVault</title>
    ...
    <main class="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div class="w-full max-w-[440px]">
            ...
            <div class="mb-8 relative group">
                <div class="absolute -inset-1 bg-gradient-to-r from-primary to-orange-400 rounded-xl blur opacity-20 group-hover:opacity-30 transition duration-1000"></div>
                <div class="relative w-full h-32 rounded-xl overflow-hidden border border-white/10 bg-slate-card">
                    <div class="absolute inset-0 bg-gradient-to-br from-primary/20 to-orange-500/10"></div>
                    <div class="absolute inset-0 flex items-center justify-center">
                        <span class="material-symbols-outlined text-6xl text-primary/30">handshake</span>
                    </div>
                    <div class="absolute bottom-4 left-4">
                        <span class="text-[10px] text-primary font-bold uppercase tracking-widest px-2 py-0.5 bg-primary/10 border border-primary/20 rounded">Collaborator Portal</span>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center mb-10">
                <h1 class="text-white text-3xl font-bold tracking-tight mb-2">Collaborator Login</h1>
                <p class="text-slate-400 text-sm">StegaVault — Collaborator Portal</p>
            </div>
            ...
            <div class="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl">
                ...
                <div class="mt-4 text-center">
                    <p class="text-xs text-slate-500 flex items-center justify-center gap-1">
                        ...
                        Collaborator credentials provided by admin
                    </p>
                </div>
            ...
            try {
                ...
                if (data.success) {
                    if (data.data && data.data.require_mfa) {
                        window.location.href = '../mfa-verify.php?redirect=collaborator/dashboard.php';
                        return;
                    }
                    ...
                    // Check if user is actually an collaborator (not admin)
